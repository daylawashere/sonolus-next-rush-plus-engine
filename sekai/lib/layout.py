...
def get_camera_info(target_time: float | None = None, left_limit: bool = False) -> CameraInfo:
    ...
    default_zoom = 1 - Options.default_camera_zoom if Options.force_dynamic_stage else 0
    default_tilt = Options.default_stage_tilt if Options.force_dynamic_stage else 1
        ...
        if camera_b_ref.index > 0:
            ...
            if camera_b.time > camera_a.time:
                ...
                result @= CameraInfo(
                    ...
                    rotate=lerp(camera_a.rotate, camera_b.rotate, p) + default_rotate,
                    stage_tilt=lerp(camera_a.stage_tilt, camera_b.stage_tilt, p) * default_tilt,
                )
                return result
        result @= CameraInfo(
            lane=camera_a.lane,
            ...
            stage_tilt=camera_a.stage_tilt * default_tilt,
        )
        return result
    if camera_b_ref.index > 0:
        camera_b = get_event_as(camera_b_ref, camera_archetype)
        result @= CameraInfo(
            ...
            rotate=camera_b.rotate + default_rotate,
            stage_tilt=camera_b.stage_tilt * default_tilt,
        )
        return result
    result @= CameraInfo(
        lane=0.0,
...
def init_ui_margin():
    ...
    match LevelConfig.ui_version:
        case Version.v3:
            ...
            UIMargin.score_bar_x = 0.3 if has_side_notch else 0.069
...
def layout_score_bar() -> Quad:
    ...
    match LevelConfig.ui_version:
        case Version.v3:
            ...
            w = h * 4.745
        case Version.v1:
            base_h_unscaled = 0.31 * ui.primary_metric_config.scale
            h = base_h_unscaled * scale_ratio
            w = h * 4.64
...
def layout_score_gauge(gauge=0.0, score_type: ScoreGaugeType = ScoreGaugeType.NORMAL) -> Quad:
    ...
    bar_h_unscaled = (
        ...
        if LevelConfig.ui_version == Version.v3
        else 0.31 * ui.primary_metric_config.scale
    )
    bar_h_current = bar_h_unscaled * scale_ratio
    y_shift = (bar_h_unscaled - bar_h_current) / 2
    ...
    match LevelConfig.ui_version:
        ...
        case Version.v1:
            ...
            y_offset = 0.024
            h = max(
                (0.048 * ui.primary_metric_config.scale * scale_ratio),
                1e-3,
            )
            w = max(
                (h * 23.5 * ((1 - gauge) if score_type == ScoreGaugeType.MASK else 1)),
                1e-3,
            )
...
def layout_note_body_by_edges(l: float, r: float, h: float, travel: float):
    match Options.note_perspective:
        ...
        case 2:
            p = (travel* Options.perspective_mult)**1.31 if LevelConfig.ui_version == Version.v3 else 1 
        case _:
            p = 1
