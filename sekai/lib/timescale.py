...
class CompositeTime(Record):
    ...
    def add(self, value: float) -> None:
        ...
        if abs(self.base) >= abs(value):
            self.delta += (self.base - t) + value
        else:
            self.delta += (value - t) + self.base
        self.base = t
...
class TimeToScaledTime(Record):
    ...
    def reset(self, first_change_index: int):
        ...
        self.next_change_index = first_change_index
        self.next_time = MIN_START_TIME
        self.next_timescale = 1.0

    def get(self, time: float, first_change_index: int) -> CompositeTime:
        ...
        if self.next_time > MIN_START_TIME and time < self.next_time:  # if caching
            ...
            match self.last_ease:
                case TimescaleEase.NONE:
                    result.add((time - self.last_time) * self.last_timescale)
                    return result
                case TimescaleEase.LINEAR:
                    ...
                    result.add((time - self.last_time) * avg_timescale)
                    return result
                case _:
        ...
        for change in iter_timescale_changes(self.next_change_index):
            ...
            next_scaled_time @= self.last_scaled_time
            match self.last_ease:
                case TimescaleEase.NONE:
                    next_scaled_time.add((next_time - self.last_time) * self.last_timescale)
                case TimescaleEase.LINEAR:
                    next_scaled_time.add((next_time - self.last_time) * (next_timescale + self.last_timescale) / 2)
                case _:
                    assert_never(self.last_ease)
            skip_scaled_time = change.cached_skip_time
            if time == next_time:
                result @= next_scaled_time
                result.add(skip_scaled_time)
                return result
            if time < next_time:
                result @= self.last_scaled_time
            ...
            self.last_scaled_time.add(skip_scaled_time)
...
class TimeToLastChangeIndex(Record):
    ...
    def get(self, time: float, first_change_index: int) -> int:
        if time < self.last_time:
            self.reset(first_change_index)
...
class ScaledTimeToFirstTime(Record):
    ...
    def get(self, scaled_time: float, first_change_index: int) -> float:
...
def iter_timescale_changes_in_group_from_time(
    ...
    next_index = group_entity.time_to_last_change_index.get(time, group_entity.first_ref.index)
    if next_index <= 0:
...
def group_time_to_scaled_time(
    ...
    if isinstance(group, EntityRef):
        group = group.index
    group_entity = timescale_group_archetype().at(group)
    return group_entity.time_to_scaled_time.get(time, group_entity.first_ref.index)
...
def group_scaled_time_to_first_time(
    ...
    if isinstance(group, EntityRef):
        group = group.index
    group_entity = timescale_group_archetype().at(group)
    return group_entity.scaled_time_to_first_time.get(scaled_time, group_entity.first_ref.index)
...
def group_scaled_time_to_first_time_2(
    ...
    if isinstance(group, EntityRef):
        group = group.index
    group_entity = timescale_group_archetype().at(group)
    return group_entity.scaled_time_to_first_time_2.get(scaled_time, group_entity.first_ref.index)
